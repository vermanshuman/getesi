...
public class RequestHelper {
    ...
    public static List<Service> onRequestTypeChange(Long requestTypeId,
                                                       Long selectedClientId, int executionType, Long subjectType)
        ...
        if (!ValidationHelper.isNullOrEmpty(requestTypeId) && !ValidationHelper.isNullOrEmpty(selectedClientId)) {
            ...
            if(executionType > 0){
                ...
                if (ValidationHelper.isNullOrEmpty(subjectType) || SubjectType.PHYSICAL_PERSON.getId().equals(subjectType)) {
                    services = DaoManager.load(Service.class,new Criterion[]{
                            Restrictions.and(Restrictions.eq("requestType.id", requestTypeId), Restrictions.eq("physicalSubject", Boolean.TRUE))
                    },  Order.asc("name"));
                }
                if (!ValidationHelper.isNullOrEmpty(subjectType) && SubjectType.LEGAL_PERSON.getId().equals(subjectType)) {
                    services = DaoManager.load(Service.class,new Criterion[]{
                            Restrictions.and(Restrictions.eq("requestType.id", requestTypeId), Restrictions.eq("giuridicSubject", Boolean.TRUE))
                    },  Order.asc("name"));
                }
            }
            ...
            List<Service> clientServiceInfo = DaoManager.loadField(ClientServiceInfo.class, "service", Service.class, new Criterion[]{
                    Restrictions.eq("client", client),
                    Restrictions.eq("visible", Boolean.TRUE)});
            if(!ValidationHelper.isNullOrEmpty(clientServiceInfo)) {
            	visibleServices = filteredServices.stream().filter(item-> clientServiceInfo.contains(item)).collect(Collectors.toList());
            }
            if(!ValidationHelper.isNullOrEmpty(visibleServices)){
...
//            }, !multiple);
    ...
    public static void updateState(Long requestId, Long stateId)
        ...
        if (!ValidationHelper.isNullOrEmpty(requestId) && !ValidationHelper.isNullOrEmpty(stateId)) {
            ...
            if (!ValidationHelper.isNullOrEmpty(request)) {
                ...
                request.setCertificationStateId(stateId);
    ...
    public static String getPdfRequestBody(Request request) {
        return "<br/>" + getFirstPart(request) + getSecondPart(request) +
...
//                }
    ...
    public static String getRequestSubjects(Request request, Boolean filter) throws PersistenceBeanException, IllegalAccessException {
        String subjectNames = "";
        List<RequestSubject> requestSubjects = DaoManager.load(RequestSubject.class,
                new Criterion[]{Restrictions.eq("request", request)});
            for(RequestSubject requestSubject : CollectionUtils.emptyIfNull(requestSubjects)) {
                if(!filter || StringUtils.isBlank(requestSubject.getType())
                        || !requestSubject.getType().equals(SectionCType.CONTRO.getName())){
                    String subject = requestSubject.getSubject().getFullName() + (subjectNames.isEmpty() ? "" : " - ");
                    subjectNames += subject.toUpperCase() + ", ";
                }
    ...
    public static Boolean checkOutputType(Request examRequest) {
        if (!ValidationHelper.isNullOrEmpty(examRequest)) {
            if (!ValidationHelper.isNullOrEmpty(examRequest.getService())) {
                if (examRequest.getService().getRequestOutputType() != null &&
                        examRequest.getService().getRequestOutputType().equals(
                                RequestOutputTypes.ONLY_FILE))
                    return false;
            } else if (!ValidationHelper.isNullOrEmpty(examRequest.getMultipleServices())) {
                Service filteredService = examRequest
                        .getMultipleServices()
