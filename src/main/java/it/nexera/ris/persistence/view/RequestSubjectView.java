...
@Immutable
@Table(name = "request_subject_new_view")
@View(sql = RequestSubjectView.QUERY)
    public class RequestSubjectView extends IndexedView {
    ...
    public static final String CREATE_PART = "CREATE OR REPLACE VIEW request_subject_new_view ";
    ...
    @Transient
    private Integer lastRequestManageBy;
...
//                Restrictions.ne("typeId", DocumentType.FORMALITY.getId())
    ...
    public Boolean getSendBrexa() throws HibernateException, InstantiationException, IllegalAccessException, PersistenceBeanException {
        if(!ValidationHelper.isNullOrEmpty(getRequestTypeId())) {
            RequestType requestType = DaoManager.get(RequestType.class, getRequestTypeId());
            if(requestType != null) {
                setSendBrexa(requestType.getSendBrexa());
            }
        }
        return sendBrexa;
    }
...
*/
    public Integer getLastRequestManageBy() {
        if (lastRequestManageBy == null) {
            try {
                List<Integer> lastRequestManageBys = DaoManager.getSession()
                        .createQuery("select managedBy from RequestManagedBy where request= :request_id order by createDate desc")
                        .setLong("request_id", getId()).list();
                if(lastRequestManageBys.size()>0) {
                    lastRequestManageBy = lastRequestManageBys.get(0);
                }
            } catch (PersistenceBeanException | IllegalAccessException e) {
                LogHelper.log(log, e);
            }
        }
        return lastRequestManageBy;
    }
   ...
   /* public Boolean getExternalBrexa() {
    ...
    public Subject getSubject() throws HibernateException, InstantiationException, IllegalAccessException, PersistenceBeanException {
    	if(!ValidationHelper.isNullOrEmpty(getSubjectId())) {
            Subject subject = DaoManager.get(Subject.class, getSubjectId());
            if(subject != null) {
                setSubject(subject);
    ...
    public RequestType getRequestType() {
    	if(!ValidationHelper.isNullOrEmpty(getRequestTypeId())) {
			...
			try {
				requestType = DaoManager.get(RequestType.class, getRequestTypeId());
				if(requestType != null) {
