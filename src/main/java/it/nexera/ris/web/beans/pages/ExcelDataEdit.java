...
import it.nexera.ris.persistence.beans.entities.domain.RequestPrint;
...
//            }
			...
			if (!ValidationHelper.isNullOrEmpty(getMail())) {
				Document document = DaoManager.get(Document.class,
						new Criterion[] { Restrictions.eq("mail.id", getMailId()) });
				...
				setExcelDate((document == null || document.getInvoiceDate() == null ? DateTimeHelper.getNow()
			...
			if (getSelectedNotManagerOrFiduciaryClientId() != null) {
				...
				setInvoiceClients(ComboboxHelper.fillList(
						invoiceClients.stream().filter(distinctByKey(c -> c.getId())).collect(Collectors.toList()),
						true));
				...
				if (!ValidationHelper.isNullOrEmpty(getSelectedNotManagerOrFiduciaryClientId())) {
					...
					if (!ValidationHelper.isNullOrEmpty(offices)) {
						setOfficeList(ComboboxHelper.fillList(offices.stream()
								.sorted(Comparator.comparing(Dictionary::getDescription)).collect(Collectors.toList()),
								true));
					} else {
			...
			// duplicate code >>> no need >>> handled below
//			if (!ValidationHelper.isNullOrEmpty(getMail())) {
...
//			}
			...
			if (!ValidationHelper.isNullOrEmpty(getMail())) {
				...
				if (!ValidationHelper.isNullOrEmpty(getMail().getClient())
					...
					setNotManagerOrFiduciaryClients(ComboboxHelper
							.fillList(notManagerOrFiduciaryClients.stream().filter(c -> Objects.nonNull(c))
				...
				if (!ValidationHelper.isNullOrEmpty(getMail())) {
					setClientFiduciary(getMail().getFiduciary());
				}
				...
				setNotManagerOrFiduciaryClients(
						ComboboxHelper.fillList(notManagerOrFiduciaryClients.stream().filter(c -> Objects.nonNull(c))
			...
			if (!ValidationHelper.isNullOrEmpty(getMail())) {
				...
				setSelectedRequests(DaoManager.load(Request.class,
						new Criterion[] { Restrictions.in("id", getSelectedRequestIds()) }));
	...
	private void prepareTables(List<Request> requests)
		...
		for (Map.Entry<RequestType, List<Request>> entry : sortedRequests.entrySet()) {
			...
			List<ClientInvoiceManageColumn> clientInvoiceManageColumns = DaoManager.load(
					ClientInvoiceManageColumn.class,
					...
					new Criterion[] { Restrictions.and(Restrictions.eq("client.id", requestClient.getId()),
			...
			for (Request request : entry.getValue()) {
				if (!ValidationHelper.isNullOrEmpty(request.getService())) {
					...
					List<ExtraCost> extraCost = DaoManager.load(ExtraCost.class,
							new Criterion[] { Restrictions.eq("requestId", request.getId()) });
					Double result;
					...
					for (Service service : request.getMultipleServices()) {
						try {
							...
							newRequest.setMultipleRequestId(request.getId());
							newRequests.add(newRequest);
							addColumnValues(newRequest, service, columnValues, createExcelRequestsReportHelper, index++,
									result);
						} catch (CloneNotSupportedException e) {
							LogHelper.log(log, e);
						}
			...
			footerValues.put(ResourcesHelper.getString("requestedDate"),
					ResourcesHelper.getString("formalityTotal").toUpperCase());
			colIndex = getIndex(ResourcesHelper.getString("mortgageRights"),
					CreateExcelRequestsReportHelper.getRequestsColumns());
			...
			double result = 0.0;
			if (colIndex > -1) {
				result = createExcelRequestsReportHelper
						.getSumOfCostEstateFormalityService(excelTableWrapper.getOriginalRequests());
				footerValues.put(ResourcesHelper.getString("mortgageRights"),
						result > 0 ? formatter.format(result) : "0.0");
			}
			colIndex = getIndex(ResourcesHelper.getString("landRegistryRights"),
					CreateExcelRequestsReportHelper.getRequestsColumns());
			if (colIndex > -1) {
				result = getSumOfCostCadastral(excelTableWrapper.getOriginalRequests());
				footerValues.put(ResourcesHelper.getString("landRegistryRights"),
						result > 0 ? formatter.format(result) : "0.0");
			}

			colIndex = getIndex(ResourcesHelper.getString("excelStamps"),
					CreateExcelRequestsReportHelper.getRequestsColumns());
			if (colIndex > -1) {
				result = getSumOfExtraCost(excelTableWrapper.getOriginalRequests(), ExtraCostType.MARCA);
				footerValues.put(ResourcesHelper.getString("excelStamps"),
						result > 0 ? formatter.format(result) : "0.0");
			}

			colIndex = getIndex(ResourcesHelper.getString("excelPostalExpenses"),
					CreateExcelRequestsReportHelper.getRequestsColumns());
			if (colIndex > -1) {
				result = getSumOfExtraCost(excelTableWrapper.getOriginalRequests(), ExtraCostType.POSTALE);
				footerValues.put(ResourcesHelper.getString("excelPostalExpenses"),
						result > 0 ? formatter.format(result) : "0.0");
			}

			colIndex = getIndex(ResourcesHelper.getString("compensation"),
					CreateExcelRequestsReportHelper.getRequestsColumns());
			if (colIndex > -1) {
				result = createExcelRequestsReportHelper
						.getSumOfCostPayServices(excelTableWrapper.getOriginalRequests());
				footerValues.put(ResourcesHelper.getString("compensation"),
						result > 0 ? formatter.format(result) : "0.0");
			}

			colIndex = getIndex(ResourcesHelper.getString("formalityTotal"),
					CreateExcelRequestsReportHelper.getRequestsColumns());
			if (colIndex > -1) {
				result = createExcelRequestsReportHelper
						.getSumOfCostTotalServices(excelTableWrapper.getOriginalRequests());
				footerValues.put(ResourcesHelper.getString("formalityTotal"),
						result > 0 ? formatter.format(result) : "0.0");
			}
	...
	private void addColumnValues(Request request, Map<String, String> columnValues,
		...
		int colIndex = getIndex(ResourcesHelper.getString("requestedDate"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("requestedDate"), request),
					request.getCreateDateStr());
		}

		colIndex = getIndex(ResourcesHelper.getString("nominative"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("nominative"), request),
					request.getSubject() != null
							? (request.getSubject().getFullName() != null
									? request.getSubject().getFullName().toUpperCase()
									: "")
							: "");
		}
		colIndex = getIndex(ResourcesHelper.getString("codFiscIva"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("codFiscIva"), request),
					request.getFiscalCodeVATNamber());
		}
		colIndex = getIndex(ResourcesHelper.getString("permissionRequest"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("permissionRequest"), request),
					request.getServiceName().toUpperCase());
		}
		colIndex = getIndex(ResourcesHelper.getString("requestListCorservatoryName"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("requestListCorservatoryName"), request),
					request.getAggregationLandChargesRegistryName());
		}
		colIndex = getIndex(ResourcesHelper.getString("requestPrintFormalityPresentationDate"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("requestPrintFormalityPresentationDate"), request),
					DateTimeHelper.toString(request.getEvasionDate()));
		}
		colIndex = getIndex(ResourcesHelper.getString("excelForm"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			Long val = request.getNumberActOrSumOfEstateFormalitiesAndOther().longValue()
					+ createExcelRequestsReportHelper.getRequestExtraCostValue(request).longValue();
		...
		colIndex = getIndex(ResourcesHelper.getString("landRegistryRights"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("landRegistryRights"), request),
					String.valueOf(getCostCadastralAndExtraCostRelated(request)));
		}

		colIndex = getIndex(ResourcesHelper.getString("excelStamps"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("excelStamps"), request),
					String.valueOf(getExtraCostRelated(request, ExtraCostType.MARCA)));
		}

		colIndex = getIndex(ResourcesHelper.getString("excelPostalExpenses"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("excelPostalExpenses"), request),
					String.valueOf(getExtraCostRelated(request, ExtraCostType.POSTALE)));
		}

		colIndex = getIndex(ResourcesHelper.getString("compensation"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("compensation"), request),
					ValidationHelper.isNullOrEmpty(request.getCostPay()) ? "0" : String.valueOf(request.getCostPay()));
		}
		colIndex = getIndex(ResourcesHelper.getString("formalityTotal"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("formalityTotal"), request),
					ValidationHelper.isNullOrEmpty(request.getTotalCost()) ? "0"
							: request.getTotalCost().replaceAll(",", "."));
		}
		colIndex = getIndex(ResourcesHelper.getString("excelNote"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			if (ValidationHelper.isNullOrEmpty(request.getCostNote())) {
				...
				if (!ValidationHelper.isNullOrEmpty(requestDocuments)) {
					if (request.getService() != null && request.getService().getUnauthorizedQuote() != null
							&& request.getService().getUnauthorizedQuote()) {
				...
				if (!isAdded && request.getUnauthorizedQuote() != null && request.getUnauthorizedQuote()) {
					note = "Preventivo non autorizzato";
				}
		...
		colIndex = getIndex(ResourcesHelper.getString("excelCDR"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("excelCDR"), request), request.getCdr());
		}

		colIndex = getIndex(ResourcesHelper.getString("excelNDG"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("excelNDG"), request), request.getNdg());
		}

		colIndex = getIndex(ResourcesHelper.getString("excelUser"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("excelUser"), request),
					request.getRequestExcelUserName());
		}
		colIndex = getIndex(ResourcesHelper.getString("excelPosition"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
	...
	private void addColumnValues(Request request, Service service, Map<String, String> columnValues,
			CreateExcelRequestsReportHelper createExcelRequestsReportHelper, int index, double extraCost)
			throws IllegalAccessException, PersistenceBeanException, InstantiationException {
		...
		int colIndex = getIndex(ResourcesHelper.getString("requestedDate"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("requestedDate"), request, service),
					request.getCreateDateStr());
		}
		...
		colIndex = getIndex(ResourcesHelper.getString("nominative"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("nominative"), request, service),
					request.getSubject() != null
							? (request.getSubject().getFullName() != null
									? request.getSubject().getFullName().toUpperCase()
									: "")
		...
		colIndex = getIndex(ResourcesHelper.getString("codFiscIva"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("codFiscIva"), request, service),
					request.getFiscalCodeVATNamber());
		}
		colIndex = getIndex(ResourcesHelper.getString("permissionRequest"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			...
			columnValues.put(getColumnName(ResourcesHelper.getString("permissionRequest"), request, service),
					serviceName);
		}
		...
		if (colIndex > -1) {
			if (index != -1) {
				columnValues.put(
						getColumnName(ResourcesHelper.getString("requestListCorservatoryName"), request, service),
						request.getAggregationLandChargesRegistryName());
			} else {
				List<AggregationLandChargesRegistry> aggregationLandChargesRegistries = DaoManager.load(
						AggregationLandChargesRegistry.class,
						new Criterion[] { Restrictions.eq("national", Boolean.TRUE) });
		...
		if (colIndex > -1) {
			columnValues.put(
					getColumnName(ResourcesHelper.getString("requestPrintFormalityPresentationDate"), request, service),
					DateTimeHelper.toString(request.getEvasionDate()));
		}
		colIndex = getIndex(ResourcesHelper.getString("excelForm"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1 && index != -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("excelForm"), request, service),
		...
		if (colIndex > -1) {
			if (!ValidationHelper.isNullOrEmpty(service)) {
				Double cost = 0d;
				if (index != -1) {
					cost = createExcelRequestsReportHelper.getCostEstateFormalityAndExtraCostRelated(request, service,
							billingClient, restrictionForPriceList);
				} else {
					cost += extraCost;
				}
		...
		if (index != -1) {
			colIndex = getIndex(ResourcesHelper.getString("landRegistryRights"),
					CreateExcelRequestsReportHelper.getRequestsColumns());
			if (colIndex > -1) {
				columnValues.put(getColumnName(ResourcesHelper.getString("landRegistryRights"), request, service),
						String.valueOf(getCostCadastralAndExtraCostRelated(request)));
			}
		}
			...
			if (colIndex > -1) {
				if (!ValidationHelper.isNullOrEmpty(service)) {
					columnValues.put(getColumnName(ResourcesHelper.getString("compensation"), request, service),
							String.valueOf(createExcelRequestsReportHelper.getCostPay(request, service, billingClient,
		...
		if (colIndex > -1) {
			if (index != -1) {
				if (!ValidationHelper.isNullOrEmpty(service)) {
					...
					result += createExcelRequestsReportHelper.getCostExtra(request, service, billingClient,
							restrictionForPriceList);
					result += createExcelRequestsReportHelper.getCostEstateFormality(request, service, billingClient,
							restrictionForPriceList);
					result += createExcelRequestsReportHelper.getCostPay(request, service, billingClient,
							restrictionForPriceList);
					result = (double) Math.round((result) * 100000d) / 100000d;
					columnValues.put(getColumnName(ResourcesHelper.getString("formalityTotal"), request, service),
							String.valueOf(result));
				}
			} else {
				columnValues.put(getColumnName(ResourcesHelper.getString("formalityTotal"), request, service),
						String.valueOf(extraCost));
			}
		...
		colIndex = getIndex(ResourcesHelper.getString("excelUser"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1) {
			columnValues.put(getColumnName(ResourcesHelper.getString("excelUser"), request, service),
					request.getRequestExcelUserName());
		}
		...
		colIndex = getIndex(ResourcesHelper.getString("excelPosition"),
				CreateExcelRequestsReportHelper.getRequestsColumns());
		if (colIndex > -1 && !ValidationHelper.isNullOrEmpty(request.getPosition())) {
			columnValues.put(getColumnName(ResourcesHelper.getString("excelPosition"), request, service),
					request.getPosition());
		}
		...
		if (colIndex > -1 && index != -1) {
			...
			Double result = getExtraCostRelated(
					!ValidationHelper.isNullOrEmpty(request.getId()) ? request.getId() : request.getReferenceId(),
					ExtraCostType.MARCA);
			result = (double) Math.round((result) * 100000d) / 100000d;
		...
		if (colIndex > -1 && index != -1) {
			Double result = getExtraCostRelated(
					!ValidationHelper.isNullOrEmpty(request.getId()) ? request.getId() : request.getReferenceId(),
					ExtraCostType.POSTALE);
			result = (double) Math.round((result) * 100000d) / 100000d;
			columnValues.put(getColumnName(ResourcesHelper.getString("excelPostalExpenses"), request, service),
					"\u20AC" + result);
		}

		Request referenceRequest = DaoManager.get(Request.class,
				!ValidationHelper.isNullOrEmpty(request.getReferenceId()) ? request.getReferenceId() : request.getId());
	...
	public Double getSumOfCostCadastral(List<Request> requests)
		...
		for (Request request : requests) {
			...
			List<ExtraCost> extraCosts = DaoManager.load(ExtraCost.class, new Criterion[] {
					Restrictions.eq("requestId", request.getId()), Restrictions.eq("type", ExtraCostType.CATASTO) });
	...
	public Double getSumOfCostEstateFormality(List<Request> requests,
		...
		for (Request request : requests) {
			...
			if (!ValidationHelper.isNullOrEmpty(request.getService())) {
				...
				List<ExtraCost> extraCosts = DaoManager.load(ExtraCost.class,
						new Criterion[] { Restrictions.eq("requestId", request.getId()),
				...
				boolean restrictionForPriceList = createExcelRequestsReportHelper.restrictionForPriceList(request);
				for (Service service : request.getMultipleServices()) {
					result += createExcelRequestsReportHelper.getCostEstateFormalityAndExtraCostRelated(request,
							service, billingClient, restrictionForPriceList);
				}
			}
		}
		return result;
	...
	public Double getSumOfExtraCost(List<Request> requests, ExtraCostType extraCostType)
			throws PersistenceBeanException, IllegalAccessException {
		Double result = 0d;
		for (Request request : requests) {
			List<ExtraCost> extraCosts = DaoManager.load(ExtraCost.class, new Criterion[] {
					Restrictions.eq("requestId", request.getId()), Restrictions.eq("type", extraCostType) });
	...
	protected Double getExtraCostRelated(Request request, ExtraCostType extraCostType)
			throws PersistenceBeanException, IllegalAccessException {
		Double result = 0d;
	...
	protected Double getCostEstateFormalityAndExtraCostRelated(Request request)
			throws PersistenceBeanException, IllegalAccessException {
		Double result = 0d;
		...
		List<ExtraCost> extraCosts = DaoManager.load(ExtraCost.class, new Criterion[] {
				Restrictions.eq("requestId", request.getId()), Restrictions.eq("type", ExtraCostType.IPOTECARIO) });
	...
	protected Double getCostCadastralAndExtraCostRelated(Request request)
			throws PersistenceBeanException, IllegalAccessException {
		Double result = 0d;
	...
	private byte[] getXlsBytes() {
		...
		try {
			...
			if (!ValidationHelper.isNullOrEmpty(getMail())
					&& !ValidationHelper.isNullOrEmpty(getMail().getRecievedInbox())
					&& !ValidationHelper.isNullOrEmpty(getMail().getRecievedInbox().getRequests())) {
			...
			if ((!ValidationHelper.isNullOrEmpty(getMail()) && !ValidationHelper.isNullOrEmpty(getMail().getRequests()))
				...
				if (!ValidationHelper.isNullOrEmpty(getMail())) {
					...
					getMail().setFiduciary(excelDataWrapper.getFiduciary());
				...
				List<Request> filteredRequests = emptyIfNull(requests).stream().filter(r -> r.isDeletedRequest())
						.collect(Collectors.toList());
				Collections.sort(filteredRequests, Comparator.comparing(r -> r.getSubject().getId()));
				excelFile = new CreateExcelRequestsReportHelper(true).convertMailUserDataToExcel(filteredRequests,
						document, excelDataWrapper);
			}
		} catch (Exception e) {
			LogHelper.log(log, e);
		}
	...
	public void generateXlsRequestCost() {
		try {
			...
			Optional<Request> evadedRequest = getSelectedRequests().stream()
					.filter(r -> r.isDeletedRequest() && !ValidationHelper.isNullOrEmpty(r.getStateId())
	...
	public void updateComboboxes() throws PersistenceBeanException, IllegalAccessException {
		...
		if (!ValidationHelper.isNullOrEmpty(selectedClient)
				&& !ValidationHelper.isNullOrEmpty(selectedClient.getTypeId())) {
			...
			setNotManagerOrFiduciaryClients(ComboboxHelper.fillList(
					clientList.stream().filter(c -> Objects.nonNull(c))
							.filter(c -> (c.getFiduciary() == null || !c.getFiduciary())
									&& (c.getManager() == null || !c.getManager()))
							.collect(Collectors.toList()),
					true));
		...
		if (!ValidationHelper.isNullOrEmpty(getMail())
				&& !ValidationHelper.isNullOrEmpty(getSelectedClientFiduciaryId())) {
	...
	protected Double getExtraCostRelated(Long requestId, ExtraCostType extraCostType)
			throws PersistenceBeanException, IllegalAccessException {
		Double result = 0d;

		List<ExtraCost> extraCosts = DaoManager.load(ExtraCost.class,
				new Criterion[] { Restrictions.eq("requestId", requestId), Restrictions.eq("type", extraCostType) });
	...
	public void viewExtraCost(boolean recalculate)
			throws PersistenceBeanException, IllegalAccessException, InstantiationException {
		setCostNote(null);
		setCostManipulationHelper(new CostManipulationHelper());
		...
		if (ValidationHelper.isNullOrEmpty(getExamRequest().getCostNote())) {
			try {
				List<Document> requestDocuments = DaoManager.load(Document.class,
						new CriteriaAlias[] { new CriteriaAlias("request", "request", JoinType.INNER_JOIN) },
						new Criterion[] { Restrictions.and(Restrictions.eq("request.id", getExamRequest().getId()),
								Restrictions.eq("typeId", 2L)) });
				boolean isAdded = Boolean.FALSE;
				if (!ValidationHelper.isNullOrEmpty(requestDocuments)) {
				...
				costNote = ValidationHelper.isNullOrEmpty(costNote)
						? new CreateExcelRequestsReportHelper().generateCorrectNote(getExamRequest())
						: costNote.concat(" ")
								.concat(new CreateExcelRequestsReportHelper().generateCorrectNote(getExamRequest()));
		...
		if (!ValidationHelper.isNullOrEmpty(getMail()) && !ValidationHelper.isNullOrEmpty(getMail().getRecievedInbox())
			...
			prepareTables(getMail().getRecievedInbox().getRequests());
				...
				&& !ValidationHelper.isNullOrEmpty(getMail().getRequests())) {
			prepareTables(getMail().getRequests());
		}
	}

	public void updateNationalCost()
		...
		Request request = DaoManager.get(Request.class,
				new Criterion[] { Restrictions.eq("id", getExamRequest().getId()) });
		...
		if (!ValidationHelper.isNullOrEmpty(getCostManipulationHelper().getIncludeNationalCost())
			...
			if (!ValidationHelper.isNullOrEmpty(request) && !ValidationHelper.isNullOrEmpty(request.getService())
				...
				getCostManipulationHelper()
						.setExtraCostOtherNote(ResourcesHelper.getString("requestServiceNationalPriceNote"));
			...
			if (!ValidationHelper.isNullOrEmpty(getCostManipulationHelper().getRequestExtraCosts())) {
				Optional<ExtraCost> nationalExtraCost = getCostManipulationHelper().getRequestExtraCosts().stream()
						.filter(ec -> ec.getType().equals(ExtraCostType.NAZIONALEPOSITIVA)).findFirst();
	...
	public void createInvoice() throws Exception {
		...
		if (!ValidationHelper.isNullOrEmpty(getRequestsConsideredForInvoice())) {
			...
			List<RequestPriceListModel> requestPriceListModels = InvoiceHelper
					.groupingItemsByTaxRate(selectedRequestList);
			getInvoiceDialogBean().setSelectedInvoiceItems(InvoiceHelper.getInvoiceItems(requestPriceListModels,
					getInvoiceDialogBean().getCausal(), selectedRequestList));
			...
			log.info("request total cost :: " + totalCost + ", total invoice :: "
					+ getInvoiceDialogBean().getAllTotalLine().doubleValue());
			if (totalCost != getInvoiceDialogBean().getAllTotalLine().doubleValue()) {
				if (!ValidationHelper.isNullOrEmpty(getInvoiceDialogBean().getInvoiceErrorMessage())) {
					getInvoiceDialogBean().setInvoiceErrorMessage(getInvoiceDialogBean().getInvoiceErrorMessage()
							+ "<br /><br />" + ResourcesHelper.getString("invoiceTotalNotMatchMessage"));
				} else {
					getInvoiceDialogBean()
	...
	/*
	 * public void setMaxInvoiceNumber() throws HibernateException { LocalDate
	 * currentdate = LocalDate.now(); int currentYear = currentdate.getYear();
	 * 
	 * Long lastInvoiceNumber = 0l; try { lastInvoiceNumber = (Long)
	 * DaoManager.getMax(Invoice.class, "id", new Criterion[]{}); } catch
	 * (PersistenceBeanException | IllegalAccessException e) { LogHelper.log(log,
	 * e); } if(lastInvoiceNumber == null) lastInvoiceNumber = 0l; String
	 * invoiceNumber = (lastInvoiceNumber + 1) + "-" + currentYear + "-FE";
	...
	public void preCheckDocument()
		...
		if (!ValidationHelper.isNullOrEmpty(getMail())
			...
			List<Invoice> invoices = DaoManager.load(Invoice.class,
					new Criterion[] { Restrictions.eq("ndg", getMail().getNdg()) });
	...
	public void preCheckDocumentOtherRequests()
		...
		if (!ValidationHelper.isNullOrEmpty(getMail().getNdg())) {
			List<Request> requests = DaoManager.load(Request.class, new Criterion[] { Restrictions.and(
					Restrictions.eq("ndg", getMail().getNdg()), Restrictions.eq("stateId", RequestState.EVADED.getId()),
	...
	public void checkDocument(boolean otherRequests)
		...
		Document document = DaoManager.get(Document.class,
				new Criterion[] { Restrictions.eq("mail.id", getMail().getId()) });
	...
	public void loadInvoiceDataAfterError()
		...
		if (!ValidationHelper.isNullOrEmpty(getInvoiceDialogBean().getSelectedInvoiceClient()))
			getInvoiceDialogBean()
					.setSelectedInvoiceClientId(getInvoiceDialogBean().getSelectedInvoiceClient().getId());
		getInvoiceDialogBean().loadInvoiceDialogData(invoice);
		executeJS("PF('invoiceDialogExcelWV').show();");
