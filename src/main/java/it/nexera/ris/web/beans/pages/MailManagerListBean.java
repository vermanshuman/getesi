...
public class MailManagerListBean extends EntityLazyListPageBean<WLGInboxShort> implements Serializable {
	...
	@Getter
	@Setter
	private String mailManagerDraftCount;

	@Getter
	@Setter
	private String mailManagerReceivedCount;

	@Getter
	@Setter
	private String mailManagerSentCount;

	@Getter
	@Setter
	private String mailManagerStorageCount;

	@Override
	public void onLoad() throws NumberFormatException, HibernateException, PersistenceBeanException,
			InstantiationException, IllegalAccessException, IOException {
		System.out.println("########### OnLoad MailManagerListBean ##############");
		...
		builder.setLength(0);
		builder.append(
				"<a href=\"#\" class=\"ui-paginator-next ui-state-default ui-corner-all\"  onclick=\"nextPage()\"");
		builder.append(" tabindex=\"0\">\n");
		builder.append("<span class=\"ui-icon ui-icon-seek-next\">N</span>\n</a>\n");
		builder.append("<a href=\"#\"");
		builder.append(
				" class=\"ui-paginator-last ui-state-default ui-corner-all\" tabindex=\"-1\" onclick=\"lastPage()\">\n");
		...
		loadMailManagerCounts();
	}

	private void loadMailManagerCounts() {
		System.out.println("########### Start loadMailManagerCounts ##############");
		Date startDate = new Date();
		mailManagerDraftCount = getMailManagerDRAFT();
		mailManagerReceivedCount = getMailManagerRECEIVED();
		mailManagerSentCount = getMailManagerSENT();
		mailManagerStorageCount = getMailManagerSTORAGE();
		Date endDate = new Date();
		long seconds = (endDate.getTime() - startDate.getTime()) / 1000;
		System.out.println("########### End loadMailManagerCounts,took: " + seconds + " seconds");
	}
	...
	private void loadFilterFromCookie() {
		...
		if (!ValidationHelper.isNullOrEmpty(value)) {
			List<Long> longList = Stream.of(value.substring(1, value.length() - 1).split(",")).map(String::trim)
		...
		if (!ValidationHelper.isNullOrEmpty(value))
			...
			setTablePage(0);
		if (getTablePage() == null || getTablePage() == 0)
			setCurrentPageNumber(1);
		else
			setCurrentPageNumber(getTablePage());
...
//                "PF('tableWV').getPaginator().setPage(" + getTablePage() + ");");
	...
	public void processManagedArchiveState() {
		if (!ValidationHelper.isNullOrEmpty(getEntityEditId())) {
			try {
				WLGInbox mail = DaoManager.get(WLGInbox.class, getEntityEditId());
	...
	public void openRequest() {
		if (getSelectedRequestId() != null) {
			RedirectHelper.goToOnlyView(PageTypes.REQUEST_EDIT, getSelectedRequestId());
		}
	}
	...
	public void changeMailType() throws PersistenceBeanException, IllegalAccessException, InstantiationException {
		if (getSelectedTypeId() != null && !ValidationHelper.isNullOrEmpty(getSelectedInboxes())) {
			...
			for (WLGInboxShort inbox : getSelectedInboxes()) {
				MailManagerHelper.changeFolder(DaoManager.get(WLGInbox.class, inbox.getId()), folder,
	...
	public void rowDblSelectListener() {
		...
		try {
			...
			if (wlgInbox.getState().equals(MailManagerStatuses.NEW.getId())) {
				...
				log.info("setting mail :: " + wlgInbox.getId() + " state to :: "
						+ MailManagerStatuses.findById(wlgInbox.getState()) + " by user:: " + getCurrentUser().getId());
				try {
			...
			RedirectHelper.goToSavePage(PageTypes.MAIL_MANAGER_VIEW, getEntityEditId(), getCurrentPageNumber());
		} catch (Exception e) {
			LogHelper.log(log, e);
		}
	}
	...
	public void sortData() {
		String tableHeader = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap()
				.get("tableHeader");
	...
	public void filterTableFromPanel() {
		...
		if (!ValidationHelper.isNullOrEmpty(getDateFrom())) {
			restrictions.add(Restrictions.ge("sendDate", DateTimeHelper.getDayStart(getDateFrom())));
		}

		if (!ValidationHelper.isNullOrEmpty(getDateTo())) {
			restrictions.add(Restrictions.le("sendDate", DateTimeHelper.getDayEnd(getDateTo())));
		}

		if (!ValidationHelper.isNullOrEmpty(getFilterEmail()) || !ValidationHelper.isNullOrEmpty(getFilterAll())) {

			List<Long> listId = null;
			try {
				listId = DaoManager.loadIds(WLGInbox.class,
						new CriteriaAlias[] { new CriteriaAlias("wlgExports", "we", JoinType.INNER_JOIN) },
						new Criterion[] { Restrictions.ilike("we.destinationPath",
								!ValidationHelper.isNullOrEmpty(getFilterAll()) ? getFilterAll() : getFilterEmail(),
								MatchMode.ANYWHERE) });
			} catch (Exception e) {
				LogHelper.log(log, e);
			}
			...
			if (!ValidationHelper.isNullOrEmpty(listId)) {
				if (!ValidationHelper.isNullOrEmpty(getFilterAll())) {
					restrictions.add(Restrictions.or(Restrictions.like("emailTo", getFilterAll(), MatchMode.ANYWHERE),
					...
					restrictions.add(Restrictions.or(Restrictions.like("emailTo", getFilterEmail(), MatchMode.ANYWHERE),
		...
		if (!ValidationHelper.isNullOrEmpty(getFilterEmailFile())) {
			try {
				listId = DaoManager.loadIds(WLGInbox.class,
						new CriteriaAlias[] { new CriteriaAlias("wlgExports", "we", JoinType.INNER_JOIN) },
						new Criterion[] {
								Restrictions.ilike("we.destinationPath", getFilterEmailFile(), MatchMode.ANYWHERE) });
			} catch (Exception e) {
				LogHelper.log(log, e);
			}
		...
		if (defaultType != null) {
			...
			case SENT:
				restrictions.add(Restrictions.eq("serverId", Long.parseLong(ApplicationSettingsHolder.getInstance()
		...
		if (SessionHelper.get("isFromMailView") != null && ((Boolean) SessionHelper.get("isFromMailView"))) {

			SessionHelper.removeObject("isFromMailView");
			LazyDataModel<WLGInboxShort> lazyDataModel = (LazyDataModel<WLGInboxShort>) SessionHelper
					.get("mailManagerLazyModel");
		...
		if (!isLoaded) {
			this.setLazyModel(new EntityLazyListModel<>(WLGInboxShort.class, restrictions.toArray(new Criterion[0]),
					new Order[] { Order.desc("sendDate") },
					new CriteriaAlias[] { new CriteriaAlias("folder", "folder", JoinType.LEFT_OUTER_JOIN) }));
		}
		getLazyModel().load((getTablePage() - 1) * getRowsPerPage(), getRowsPerPage(), getTableSortColumn(),
				(getTableSortOrder() == null || getTableSortOrder().equalsIgnoreCase("DESC")
						|| getTableSortOrder().equalsIgnoreCase("UNSORTED")) ? SortOrder.DESCENDING
								: SortOrder.ASCENDING,
				new HashMap<>());
			...
			for (WLGInboxShort inbox : getSelectedInboxes()) {
				...
				if (mail != null) {
					if (mail.getPreviousState() != null) {
						...
						log.info("setting mail :: " + mail.getId() + " state to :: "
								+ MailManagerStatuses.findById(mail.getState()) + " by user:: "
								+ getCurrentUser().getId());
	...
	public boolean getSelectedCanActivateProcedure() {
		return !getSelectedInboxes().isEmpty()
				&& getSelectedInboxes().stream().allMatch(WLGInboxShort::getCanActivateProcedure);
	}
					...
					if (mail != null) {
						...
						if (MailManagerStatuses.READ == status) {
							...
							List<ReadWLGInbox> readWLGInboxList = DaoManager.load(ReadWLGInbox.class,
									new Criterion[] { Restrictions.eq("mailId", mail.getId()),
	...
	private void deleteMail(List<ClientEmailWrapper> mails) {
		for (int i = 0; i < mails.size(); ++i) {
			if (mails.get(i).getClientEmail().getId().equals(getEmailDeleteId())) {
				try {
	...
	private String getMailTitleValue(MailManagerTypeWrapper typeWrapper) {
		if (typeWrapper.getType().isAmount()) {
			return typeWrapper
					.getFacesTitle(String.valueOf(getSessionBean().getMailCountByTypeAndStatus(typeWrapper.getType())));
		}
	...
	public void globalSearch() throws PersistenceBeanException, IllegalAccessException, InstantiationException {
		if (!ValidationHelper.isNullOrEmpty(this.globalFilter)) {
			...
			Criterion r = Restrictions.or(Restrictions.like("emailTo", this.globalFilter, MatchMode.ANYWHERE),
			...
			loadList(WLGInboxShort.class, rest.toArray(new Criterion[0]), new Order[] { Order.desc("sendDate") },
					new CriteriaAlias[] { new CriteriaAlias("folder", "folder", JoinType.LEFT_OUTER_JOIN) });
		} else {
	...
	public void onPageChange() {
		String rowsPerPage = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap()
				.get("table_rppDD");
		String pageNumber = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap()
				.get("pageNumber");
...
//                setPaginatorString(builder.toString());
				builder.setLength(0);
				builder.append(
						"<a href=\"#\" class=\"ui-paginator-next ui-state-default ui-corner-all ui-state-disabled\"");
				builder.append(" tabindex=\"0\">\n");
				builder.append("<span class=\"ui-icon ui-icon-seek-next\">N</span>\n</a>\n");
				builder.append("<a href=\"#\"");
				builder.append(
						" class=\"ui-paginator-last ui-state-default ui-corner-all ui-state-disabled\" tabindex=\"-1\">\n");
				builder.append("<span class=\"ui-icon ui-icon-seek-end\">E</span>\n</a>\n");
				setPageNavigationEnd(builder.toString());
			} else {
				builder.setLength(0);
...
//                }
	...
	public void handleRowsChange() {
		String rowsPerPage = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap()
				.get("rowsPerPageSelected");
		// String rowsPerPage =
		// FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().get("table_rppDD");
		String pageNumber = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap()
				.get("pageNumber");
		if (!ValidationHelper.isNullOrEmpty(rowsPerPage))
			setRowsPerPage(Integer.parseInt(rowsPerPage));
...
//        }
	...
	public void selectAllInbox() throws PersistenceBeanException, InstantiationException, IllegalAccessException {
		String selectedIds = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap()
				.get("selectedIds");
	...
	public String getSelectedIds() {
		return CollectionUtils.emptyIfNull(getSelectedInboxes()).stream().map(WLGInboxShort::getStrId)
	...
	public void handleParentCheck() {
		if (!ValidationHelper.isNullOrEmpty(getParentCheck()) && getParentCheck()) {
			((List<WLGInboxShort>) getLazyModel().getWrappedData()).stream().forEach(w -> w.setRowCheck(Boolean.TRUE));
			getSelectedInboxes().addAll((List<WLGInboxShort>) getLazyModel().getWrappedData());
		} else {
			((List<WLGInboxShort>) getLazyModel().getWrappedData()).stream().forEach(w -> w.setRowCheck(Boolean.FALSE));
			setSelectedInboxes(new ArrayList<>());
		}
	}
