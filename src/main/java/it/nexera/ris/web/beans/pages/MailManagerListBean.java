...
public class MailManagerListBean extends EntityLazyListPageBean<WLGInboxShort> implements Serializable {
    ...
    @Getter
    @Setter
    private List<Criterion> filterTableCriteria;
    ...
    @Getter
    @Setter
    private List<SelectItem> searchActivities;

    @Getter
    @Setter
    private Long[] selectedSearchActivityIds;

    @Getter
    @Setter
    private  List<Criterion> dashboardRestrictions;
    ...
    public void onLoad() throws NumberFormatException, HibernateException,
        ...
        String mailToBeSentOnly = (String) SessionHelper.get("MAIL_MANAGER_LIST_TO_BE_SENT");
        if(StringUtils.isNotBlank(mailToBeSentOnly) && Boolean.parseBoolean(mailToBeSentOnly)) {
            setFromDashBoard(Boolean.TRUE);
            Object obj = SessionHelper.get("MAIL_MANAGER_LIST_TO_BE_SENT_R");
            if(obj != null){
                setDashboardRestrictions((List<Criterion>)obj);
            }else
                setDashboardRestrictions(null);
        }else{
            setFromDashBoard(Boolean.FALSE);
            setDashboardRestrictions(null);
        }
        ...
        setSearchActivities(ComboboxHelper.fillList(Activity.class, false));
...
//                ));
//
//                List<WLGInbox> mailsWithEvaded = DaoManager.load(WLGInbox.class,
//                        new CriteriaAlias[]{
//                                new CriteriaAlias("requests", "r", JoinType.INNER_JOIN)
//                        },
//                        new Criterion[]{
...
//                                Restrictions.or(
//                                        Restrictions.eq("r.isDeleted", Boolean.FALSE),
//                                        Restrictions.isNull("r.isDeleted"))
//                        }
//                );
//                for(WLGInbox wlgInbox : mailsWithEvaded){
//                    if(!dashboardDataIds.contains(wlgInbox.getId())){
//                        Request filteredRequest = wlgInbox.getRequests()
//                                .stream()
//                                .filter(r -> r.isDeletedRequest() && r.getStateId() != null
//                                        && !r.getStateId().equals(RequestState.EVADED.getId()))
//                                .findFirst().orElse(null);
//                        if(filteredRequest == null){
//                            dashboardDataIds.add(wlgInbox.getId());
//                        }
//                    }
//                }
...
//                }
                setFilterTableCriteria(getDashboardRestrictions());
                Arrays.stream(getDashboardRestrictions().toArray(new Criterion[0]))
                        .forEach(r -> LogHelper.debugInfo(log, "Mail Manager Restriction(Dashboard) : " + r));
                this.setLazyModel(new EntityLazyListModel<>(WLGInboxShort.class, getDashboardRestrictions().toArray(new Criterion[0]),
                        new Order[]{
                                Order.desc("sendDate")
                        }, new CriteriaAlias[]{
        ...
        List<Long> activitiesMailIdList = new ArrayList<>();
        if (!ValidationHelper.isNullOrEmpty(getSelectedSearchActivityIds())) {
            List<Activity> states = Activity.getActivities(this.getSelectedSearchActivityIds());
            try {
                activitiesMailIdList = DaoManager.loadField(WLGInbox.class, "id", Long.class, new Criterion[]{
                        Restrictions.eq("user.id", UserHolder.getInstance().getCurrentUser().getId()),
                        Restrictions.in("activity", states)
                });
            } catch (Exception e) {
                LogHelper.log(log, e);
            }
            restrictions.add(Restrictions.in("id", activitiesMailIdList));
        }
        ...
        String resetLoaded = (String) SessionHelper.get("resetMailLoaded");
        if(StringUtils.isNotBlank(resetLoaded) && Boolean.parseBoolean(resetLoaded)) {
            isLoaded = false;
            SessionHelper.removeObject("resetMailLoaded");
        }
        if (!isLoaded) {
            setFilterTableCriteria(restrictions);
            Arrays.stream(restrictions.toArray(new Criterion[0]))
                    .forEach(r -> LogHelper.debugInfo(log, "Restriction : " + r));
    ...
    public void clearFiltraPanel() {
        ...
        setSelectedSearchActivityIds(null);
        setFilterAll(null);
        filterTableFromPanel();
    }
