...
public class LandChargesRegistryListBean extends
    ...
    public void onProvinceTransfer(Boolean uiChange) {
        List<City> currentTargets = null;
        if (getCities() == null) {
            setCities(new DualListModel<>(new ArrayList<>(), new ArrayList<>()));
        }

        if(uiChange)
            currentTargets = getCities().getTarget();
        ...
        if (getProvinces() != null && !ValidationHelper.isNullOrEmpty(getProvinces().getTarget())) {
            try {
                ...
                if(!ValidationHelper.isNullOrEmpty(currentTargets)){
                    Predicate<City> notContains = item -> !sourceCity.contains(item);
                   currentTargets.removeIf(notContains);
                }
            ...
            getCities().setSource(new ArrayList<>());
            currentTargets = new ArrayList<>();
        }

        if(currentTargets == null)
            currentTargets = new ArrayList<>();
        getCities().setTarget(currentTargets);
    }
        ...
        if (this.getCanEdit()) {
            ...
            if (!ValidationHelper.isNullOrEmpty(this.getEntityEditId())) {
                try {
                    ...
                    List<Province> targetProvince = new ArrayList<>();
                    if(!Hibernate.isInitialized(getEntity().getProvinces()))
                        Hibernate.initialize(getEntity().getProvinces());
                    if (!ValidationHelper.isNullOrEmpty(getEntity().getProvinces())) {
                        targetProvince = getEntity().getProvinces();
                        targetProvince.stream()
                                .filter(sourceProvince::contains)
