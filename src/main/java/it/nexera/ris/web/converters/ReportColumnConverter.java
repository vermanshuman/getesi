...
@FacesConverter(value = "reportColumnConverter")
public class ReportColumnConverter implements Converter {
	
	public transient final Log log = LogFactory.getLog(ReportColumnConverter.class);
				...
				if(id == -1 ) {
					for (BillingTypeFields field : BillingTypeFields.values()) {
						if(field.toString().equalsIgnoreCase(value)) {
							ReportColumnWrapper invoiceColumnWrapper = new ReportColumnWrapper(field);
							invoiceColumnWrapper.setSelected(Boolean.TRUE);
							return invoiceColumnWrapper;
						}
					}
				}else {
					ReportColumn clientInvoiceManageColumn = DaoManager.get(ReportColumn.class, new Criterion[]
							{Restrictions.eq("id", Long.parseLong(value))});
					if(clientInvoiceManageColumn != null) {
						ReportColumnWrapper invoiceColumnWrapper = new ReportColumnWrapper();
						invoiceColumnWrapper.setSelected(Boolean.TRUE);
